...
export const httpUpdateCluster = async (req, res) => {
  try {
    const updated = await Cluster.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
    });
...
export const httpGetAllClusterValues = async (req, res) => {
  const clusterValues = await ClusterValue.find().populate("cluster");
...
export const httpUpdateClusterValue = async (req, res) => {
  try {
    const updated = await ClusterValue.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true }
    );
  ...
  try {
    await session.withTransaction(async () => {
      if (cvOps.delete && cvOps.delete.length > 0) {
        const idsToDelete = cvOps.delete
          .map((op) => op.id)
          .filter((id) => mongoose.Types.ObjectId.isValid(id));
        if (idsToDelete.length > 0) {
          await ClusterValue.deleteMany(
            { _id: { $in: idsToDelete } },
            { session }
          );
      ...
      if (clusterOps.delete && clusterOps.delete.length > 0) {
        const idsToDelete = clusterOps.delete
          .map((op) => op.id)
          .filter((id) => mongoose.Types.ObjectId.isValid(id));
        if (idsToDelete.length > 0) {
          await ClusterValue.deleteMany(
            { cluster: { $in: idsToDelete } },
            { session }
          );
      ...
      if (clusterOps.create && clusterOps.create.length > 0) {
        for (const op of clusterOps.create) {
          ...
          results.createdClusters.push({
            tempId: op.tempId,
            newCluster: newCluster.toObject(),
          });
        }
      }
        ...
        for (const op of cvOps.create) {
          ...
          if (clusterFk && tempIdToNewIdMap.has(clusterFk)) {
            ...
            results.errors.push({
              ...
              message: `ClusterValue creation error: Temp cluster ID ${clusterFk} not resolved.`,
              item: op,
            });
            continue;
          }
          ...
          if (
            !cvDataForDb.cluster ||
            !mongoose.Types.ObjectId.isValid(cvDataForDb.cluster)
          ) {
            results.errors.push({
              type: "CREATE_CV_ERROR",
              message: `ClusterValue creation error: Invalid or missing cluster ID for ${cvDataForDb.name}. Cluster ID was: ${cvDataForDb.cluster}`,
              item: op,
            });
            continue;
          }
          ...
          results.createdClusterValues.push({
            tempId: op.tempId,
            newClusterValue: newClusterValue.toObject(),
          });
        }
      }
      ...
      if (clusterOps.update && clusterOps.update.length > 0) {
        for (const op of clusterOps.update) {
          if (op.id && mongoose.Types.ObjectId.isValid(op.id)) {
            ...
            if (updatedCluster)
              results.updatedData.clusters.push(updatedCluster.toObject());
            else
              results.errors.push({
                type: "UPDATE_CLUSTER_ERROR",
                message: `Cluster with id ${op.id} not found for update.`,
                item: op,
              });
          } else if (op.id && op.id.startsWith("temp_")) {
            results.errors.push({
              type: "UPDATE_CLUSTER_ERROR",
              message: `Cannot update cluster with temporary ID ${op.id}. It should have been created.`,
              item: op,
            });
          }
        }
          ...
          if (op.id && mongoose.Types.ObjectId.isValid(op.id)) {
            ...
            if (clusterFk && tempIdToNewIdMap.has(clusterFk)) {
              ...
              results.errors.push({
                ...
                message: `ClusterValue update error: Temp cluster ID ${clusterFk} not resolved.`,
                item: op,
              });
              continue;
            } else if (
              clusterFk &&
              !mongoose.Types.ObjectId.isValid(clusterFk)
            ) {
              results.errors.push({
                type: "UPDATE_CV_ERROR",
                message: `ClusterValue update error: Invalid cluster ID ${clusterFk}.`,
                item: op,
              });
              continue;
            } else {
            ...
            const updatedCv = await ClusterValue.findByIdAndUpdate(
              op.id,
              updatePayload,
              { new: true, runValidators: true, session }
            );
            if (updatedCv)
              results.updatedData.clusterValues.push(updatedCv.toObject());
            else
              results.errors.push({
                type: "UPDATE_CV_ERROR",
                message: `ClusterValue with id ${op.id} not found for update.`,
                item: op,
              });
          } else if (op.id && op.id.startsWith("temp_")) {
            results.errors.push({
              type: "UPDATE_CV_ERROR",
              message: `Cannot update ClusterValue with temporary ID ${op.id}. It should have been created.`,
              item: op,
            });
          }
        }
    ...
    res.status(500).json({
      error: "Bulk operation failed. Changes rolled back.",
...
export const httpUpdateADProject = async (req, res) => {
  try {
    const updated = await ADProject.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
    });
    res.status(200).json(updated);
  } catch (err) {
